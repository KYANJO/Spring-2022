...
%
function [pstar, iter] = lm(CD,CM,func, jac, p0, tol, maxiter)

%  Initialize p and oldp.
p = p0;
mprior = p0;
fp = norm(feval(func,CD,CM, mprior, p), 2)^2;
oldp = p0 * 2;
oldfp = fp * 2;
...
while (iter <= maxiter)
    ...
    % Compute rhs=-J'*f
    rhs = -J' * feval(func, CD, CM,mprior,p);
    ...
    %
    myrhs = [-feval(func, CD,CM,mprior,p); zeros(length(p), 1)];
    s = [J; sqrt(lambda) * eye(length(p))] \ myrhs;
    
    % compute the new chisq value
    fpnew = norm(feval(func, CD, CM,mprior,p+s), 2)^2;
